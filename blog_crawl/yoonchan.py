...
response = requests.get("https://yoonchan1121.tistory.com")
html = response.text
soup = BeautifulSoup(html, "html.parser")

titles = soup.select('.title_post')
blog_title = []
for title in titles:
  blog_title.append(title.text.strip())

summation = soup.select('.txt_post')
blog_summary = []
...
articles = soup.select('.box_contents .link_title')
blog_article = []
for article in articles:
  blog_article.append("https://yoonchan1121.tistory.com" + article['href'])
