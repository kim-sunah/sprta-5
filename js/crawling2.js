...
for(let i =1;i<=3;i++){
    database.ref(`선아/${i}`).on("value", snapshot => {
        ...
        document.querySelector(`.sunah${i}`).innerHTML =
            `제목 : ${title}<br>
...
for(let i =1;i<=3;i++){
    database.ref(`지원/${i}`).on("value", snapshot => {
        ...
        const {날짜:date, 내용: contents, 링크: links, 제목: title} = data;
        lastIndex = contents.indexOf('.')+1;
        document.querySelector(`.jiwon${i}`).innerHTML =
            `제목 : ${title}<br>
            내용 : ${contents.slice(0,lastIndex)} <br>
            날짜 : ${date} <br>
            링크 : ${links}`;
...
for(let i =1;i<=2;i++){
    database.ref(`태영/${i}`).on("value", snapshot => {
        const data = snapshot.val();
        const {날짜:date, 내용: contents, 링크: links, 제목: title} = data;
        document.querySelector(`.taeyoung${i}`).innerHTML =
            `제목 : ${title}<br>
            내용 : ${contents} <br>
            날짜 : ${date} <br>
            링크 : ${links}`;
...
for(let i =1;i<=3;i++){
    database.ref(`유덕/${i}`).on("value", snapshot => {
        ...
        const {날짜:date, 내용: contents, 링크: links, 제목: title} = data;
        lastIndex = contents.indexOf('.')+1;
        document.querySelector(`.yoodeock${i}`).innerHTML =
            `제목 : ${title}<br>
            내용 : ${contents.slice(0,lastIndex)} <br>
            날짜 : ${date} <br>
            링크 : ${links}`;
...
for(let i =1;i<=3;i++){
    database.ref(`윤찬/${i}`).on("value", snapshot => {
        const data = snapshot.val();
        const {날짜:date, 내용: contents, 링크: links, 제목: title} = data;
        document.querySelector(`.yoonchan${i}`).innerHTML =
            `제목 : ${title}<br>
            내용 : ${contents} <br>
            날짜 : ${date} <br>
            링크 : ${links}`;
